...
addpath("./hk_utils/"); 
addpath('~/Documents/repositories/Base_code/colormaps/colormaps_ander_biguri'); % Nice color maps. Code should still run without this. 


run_github_version = true; % If true, access included example results at TA KMSC
...
show_exixi  = false; % Show ratio of E with xi included versus E with xi = 1. 
...
% 
% stas_nrf = nan(length(sta_name_all),1); 
...
if ~ (vs_model_style == "mcmc2023"); % Not using our mcmc results? define your own vs, xi, etc. values, and get receiver functions from ears. 
    ...
    else % Load condensed data file that is present in Github for example. 
